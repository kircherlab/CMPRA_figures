...
if (length(args) == 0) {
  ...
  rds <- "results/manuscript_figures/figure3/lichic_chip_including_noeffect_OE.rds"
} else {
  upbed <- args[1]
  downbed <- args[2]
  experiment <- args[3]
...
# #specific_experiments <- c("RAD21", "CTCF", "YY1", "ZBTB21", "FOXA1", "SMC1A", "SMC3", "STAG1", "NIPBL")
...
mid_data <- lichic_chip_OE@multiOverlaps$No_Effect_OE %>%
  mutate(regulation = "NoEffect")  # Add a column to label the dataset
...
# ggplot(combined_data, aes(x = norm_zscore, y = reorder(name, norm_zscore), fill = regulation)) +
#   geom_point(
#     aes(
